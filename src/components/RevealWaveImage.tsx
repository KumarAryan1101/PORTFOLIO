...
function ImagePlane({
  ...
  // contain-fit: the whole image is always visible inside the canvas,
  // whatever shape the container is (letterboxed by the container bg)
  const scale = useMemo<[number, number, number]>(() => {
    const w = Math.min(viewport.width, viewport.height * aspectRatio);
    return [w / 2, w / aspectRatio / 2, 1];
